#!/usr/bin/env bash
# macd — Macd reference tool. Use when working with macd in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
macd v$VERSION — Macd Reference Tool
...
## What is Macd?
Macd (macd) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with macd.

## Key Concepts
- Core macd principles and fundamentals
- How macd fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Macd Matters
Understanding macd is critical for:
...
## Getting Started
1. Understand the basic macd concepts
...
cmd_formulas() {
    cat << 'EOF'
# Macd — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Macd — Instruments & Tools Overview

## Primary Instruments
- Core tools used in macd operations
...
cmd_glossary() {
    cat << 'EOF'
# Macd — Key Terms & Definitions

## Core Terminology
- **Macd**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "macd v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: macd help"; exit 1 ;;
esac
