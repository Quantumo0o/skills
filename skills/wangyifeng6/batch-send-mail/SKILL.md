...
| 邮箱                  | variable1 | variable2 |
|----------------------|-----------|-----------|
| alice@example.com    | Alice     | 产品A     |
| bob@example.com      | Bob       | 产品B     |

- **第一列**：必须是邮箱地址
- **第二列**：替换模板中的 `{variable1}`
- **第三列**：替换模板中的 `{variable2}`
- **以此类推...**
...
- 模板中使用 `{variable1}` 对应表格第二列
- `{variable2}` 对应第三列，以此类推
- 你也可以使用 `{v1}`, `{v2}...` 作为简写
- 如果模板中变量数量多于表格列数，缺失的变量会被替换为空字符串
- 如果表格列数多于模板变量数量，多余列会被忽略
