...
name: quant-aio
version: 2.1.0
description: 🐉 波龙股神量化交易系统 V2.1 — A股/期货一站式量化工作台。多因子选股 · A股回测(T+1/涨跌停) · 金字塔建仓策略 · 华鑫QMT实盘 · 文华财经期货 · 实时风控告警。
author: SimonsTang
license: MIT
...
**文件：** `scripts/pyramid_strategy.py`

**回测结果（2024-01-01 ~ 2025-03-28）：**
| 指标 | 数值 |
|------|------|
| 年化收益 | **+25.5%** |
| 最大回撤 | **11.7%** |
...
**核心特点：**
- 7只强势股池（比亚迪、宁德时代、招商银行、美的集团、农业银行、中国平安、中国稀土）
- 金字塔分批建仓：首次25%，加仓20%
- 马丁加仓：下跌>10%才加仓，只加一次
- 移动止盈：盈利10%后回撤8%止盈，盈利20%后回撤10%止盈

**运行方式：**
```bash
python scripts/pyramid_strategy.py --start 2024-01-01 --end 2025-03-28
```
...
| 策略文件 | 年化收益 | 回撤 | 胜率 | 推荐度 |
|----------|----------|------|------|--------|
| pyramid_strategy.py | +25.5% | 11.7% | 60.7% | ⭐⭐⭐⭐⭐ 最优 |
| real_backtest.py | +13.3% | 5.4% | 61.5% | ⭐⭐⭐⭐ |

**注意：** 目标"年化30%+回撤5%"（卡玛比率6.0）在纯股票多头策略中接近不可能。当前最优策略卡玛比率2.18，已是优秀的A股多头策略表现。
...
### v2.1.0 (2026-03-28)
- 新增金字塔分批建仓策略（pyramid_strategy.py）
- 新增马丁加仓策略（下跌>10%才加仓，只加一次）
- 优化股票池：精选7只强势股
- 真实数据回测验证：年化+25.5%，回撤11.7%
- 新增双策略系统（dual_strategy.py）
- 新增多策略轮动系统（multi_strategy.py）
