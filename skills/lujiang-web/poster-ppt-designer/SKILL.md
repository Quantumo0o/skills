...
如果用户没有灵雀AI账号密码，需要帮助用户自动注册。自动注册流程如下：

### 自动注册流程

1. **获取用户手机号**：让用户输入手机号
2. **打开灵雀AI网站**：https://lqai.net/
3. **点击登录**：点击右上角的"登录"按钮
4. **选择短信登录**：在登录页面选择"短信登录"方式
5. **输入手机号**：在输入框中填写用户的手机号
6. **获取验证码**：点击"获取验证码"按钮
7. **等待用户给验证码**：告知用户"请告诉我发送到您手机上的验证码"
8. **输入验证码**：用户提供的验证码后，在验证码输入框中填写
9. **点击登录/注册**：点击"登录/注册"按钮完成注册和登录

### 自动注册代码

技能目录 `~/.openclaw/workspace/skills/poster-ppt-designer/` 下包含自动注册脚本 `auto_register.py`，可自动完成上述注册流程。

```bash
cd ~/.openclaw/workspace/skills/poster-ppt-designer

# 运行自动注册脚本（需要用户在浏览器中配合操作）
python auto_register.py
```

### 自动注册注意事项

- 自动注册需要用户在浏览器中配合操作（查看验证码、点击按钮等）
- 注册完成后，账号密码会自动保存到 `config.json` 文件中
- 后续使用无需再次注册，直接使用配置的账号密码即可