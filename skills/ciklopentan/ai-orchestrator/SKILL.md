...
| `--think` | Enable DeepThink mode (deeper reasoning) |
...
# 2. DeepSeek actually responds?
ask-deepseek.sh --dry-run || echo "DeepSeek not ready"  # Checks auth + composer; dry-run now works without explicit text

# 3. Full test (optional)
ask-deepseek.sh "Say OK" --daemon  # Should return "OK"
```
...
|---|---|---|
| `Connection refused`, `ECONNREFUSED`, daemon not online | `pm2 restart deepseek-daemon && sleep 8 && ask-deepseek.sh --dry-run` | `ask-deepseek.sh "Say OK" --daemon` |
| `auth expired`, `CAPTCHA`, login page, `401`, `403` | `pm2 stop deepseek-daemon && rm -f .daemon-ws-endpoint && ./ask-deepseek.sh "auth" --visible --wait --dry-run && pm2 start deepseek-daemon` | `ask-deepseek.sh --dry-run` |
...
| `lock`, `Singleton`, profile already in use | `rm -f .profile/Singleton* && pm2 restart deepseek-daemon` | `ask-deepseek.sh --dry-run` |
...
| 2 | DeepSeek unreachable | Check network, pm2 status |
