...
const { getMbusaDealers, getMbusaInventory } = require('./src/tool.js'); 
...
app.get('/api/dealers', async (req, res) => {
    try {
        ...
        if (!zip) {
            return res.status(400).json({ status: "error", message: "Missing required parameter: zip" });
        }
        ...
        if (resultJson.status === "error") {
            return res.status(502).json(resultJson); 
        }
...
// ==========================================
// ENDPOINT: Vehicle Inventory Search
// ==========================================
app.get('/api/inventory', async (req, res) => {
    try {
        ...
        // Map query parameters to the params object expected by the tool
        const params = {
            zip: zip,
...
app.listen(PORT, () => {
    ...
    console.log(`Test Inventory: http://localhost:${PORT}/api/inventory?zip=10019&distance=50`);
});