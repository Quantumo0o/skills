...
description: 章节大纲生成器 - 基于 15 节拍系统生成小说章节大纲。当需要根据故事大纲和章节号创建详细写作大纲时使用，支持字数分配、角色参考、风格注入、章节类型检测、三幕结构标注。
---
...
根据小说项目文件（outline.md, style.yml, characters/）和章节号，自动生成结构化的 15 节拍章节大纲，包含字数分配、角色参考、风格备注、章节类型检测和三幕结构标注。

**2026-04-13 更新**：
- ✅ 新增章节类型检测（角色引入/冲突升级/情感转折/推理探案/世界观展开/日常过渡）
- ✅ 新增三幕结构标注（第一幕：设定/第二幕：对抗/第三幕：解决）
- ✅ 节拍描述根据章节类型动态生成，不再是空模板
- ✅ 保留 15 节拍框架和字数分配
...
**无需外部 API**：所有章节分析基于规则实现，不依赖 LLM 或外部服务。
