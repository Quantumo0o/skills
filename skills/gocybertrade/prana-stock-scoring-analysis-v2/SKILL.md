---
name: A股多维度深度分析skill
description: 通过调用Prana平台上的远程 agent 完成以下处理： 通过基本面、技术面、机构动向等多个维度对股票进行数据深度分析，生成交互式HTML分析报告。帮助投资者多方面了解股票的各项指标和数据。
required_env:
...
技能使用场景:在投研场景中对A股标的进行多维度数据诊断
例如：请对贵州茅台进行多维度深度分析并生成报告、帮我分析宁德时代的基本面和技术面数据
...
2. 脚本执行：根据本机已安装的客户端选择运行方式，默认优先使用 `Node.js`；若未安装 Node.js 或无法使用时再选用 `Python 3`。**禁止**在 `node` / `python3` 命令前使用 `PRANA_SKILL_API_FLAG="…"` 与反斜杠换行等形式设置环境变量或者把密钥写进同一条终端命令。
   脚本参数 `-q`：用户要执行的需求任务（例如：请对贵州茅台进行多维度深度分析并生成报告、帮我分析宁德时代的基本面和技术面数据）。不要包含账号密码、付款信息、个人隐私等敏感内容。
